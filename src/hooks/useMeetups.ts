...
interface UseMeetups {
  myId: string | null;
  /** Pending requests where I'm the host (someone wants to join my status). */
  incoming: JoinActivity[];
  /** Lookup my own request state for a given status id. */
  outgoingState: (statusId: string) => JoinState | undefined;
  requestJoin: (statusId: string) => Promise<{ ok: boolean; message?: string }>;
  respond: (requestId: string, accept: boolean) => Promise<{ ok: boolean; message?: string }>;
  refresh: () => Promise<void>;
}

/** Owns join requests + meetup state for the current user. */
export function useMeetups(enabled: boolean): UseMeetups {
      ...
      if (error) {
        return {
          ...
          message: error.message.includes('own_status')
            ...
            : error.message.includes('status_unavailable')
