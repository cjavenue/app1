...
import { Clock, NavigationArrow, Cards, X, ChatCircle } from '@phosphor-icons/react';
...
function Card({ status }: { status: NearbyStatus }) {
  ...
  return (
    <div className="card" style={{ position: 'absolute', inset: 0, borderRadius: 24, padding: 22, display: 'flex', flexDirection: 'column' }}>
      <div style={{ position: 'absolute', right: -24, bottom: -24, opacity: 0.05, color: 'var(--teal)' }}>
        <cat.Glyph size={220} weight="fill" />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: 22, background: 'var(--teal)', color: '#0F1316', display: 'grid', placeItems: 'center', fontWeight: 800 }}>
    ...
    <div className="screen" style={{ display: 'flex', flexDirection: 'column', padding: '0 20px' }}>
      ...
      {top && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 28, paddingBottom: 16 }}>
          <button onClick={skip} className="fab" style={{ width: 64, height: 64, background: 'var(--elevated)', color: 'var(--danger)' }}>
            <X size={26} />
          </button>
          <button onClick={connect} className="fab" style={{ width: 70, height: 70, background: 'linear-gradient(120deg,var(--teal),var(--yellow))', color: '#0F1316' }}>
